...
from babyagi import (LOAD_SKILLS, api_keys, execute_skill, execute_task_list,
                     get_skills)
...
def combine_summaries(overall, latest):
    ...
    completion = openai.ChatCompletion.create(
        ...
        messages=[
            ...
            {
                "role": "user",
                "content": f"Overall summary: {overall}\nLatest summary: {latest}",
            },
        ],
    )
...
def openai_function_call(user_message):
        ...
        functions=[
            {
                ...
                "description": "Determine whether to respond via ChatCompletion, use a skill, or create a task list. Always provide a message_to_user.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "message_to_user": {
                            "type": "string",
                            "description": "A message for the user, indicating the AI's action or providing the direct chat response. ALWAYS REQUIRED. Do not use line breaks.",
                        },
                        "path": {
                            "type": "string",
                            "enum": [
                                ...
                                "Skill",
                                "TaskList",
                            ],  # Restrict the values to these three options
                            "description": "The type of response – either 'ChatCompletion', 'Skill', or 'TaskList'",
                        },
                        "skill_used": {
                            "type": "string",
                            "description": f"If path is 'Skill', indicates which skill to use. If path is 'Skill', ALWAYS use skill_used. Must be one of these: {global_skill_registry}",
                        },
                        "objective": {
                            "type": "string",
                            "description": "If path is 'Skill' or 'TaskList', describes the main task or objective. Always include if path is 'Skill' or 'TaskList'.",
                        },
                    },
    ...
    if path == "Skill":
        ...
        threading.Thread(
            target=execute_skill, args=(skill_used, objective, task_id)
        ).start()
        ...
        threading.Thread(
            target=execute_task_list, args=(objective, api_keys, task_id)
        ).start()
        update_ongoing_tasks_file()
...
def determine_response():
    try:
        ...
        ai_entry = {"role": "assistant", "content": response_data["message_to_user"]}
