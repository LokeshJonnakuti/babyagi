...
REFLECTION = (
    False  # Experimental reflection step between each task run (when running tasklist)
)


def run_single_task(
    task_id, task, skill_registry, task_outputs, OBJECTIVE, task_registry
):
    """Execute a single task and update its status"""
    task_output = task_registry.execute_task(
        task_id, task, skill_registry, task_outputs, OBJECTIVE
    )

    task_outputs[task_id]["output"] = task_output
    task_outputs[task_id]["completed"] = True
    task_outputs[task_id]["description"] = task.get(
    ...
    if task_output:
        task_registry.update_tasks(
            {"id": task_id, "status": "completed", "result": task_output}
        )

        completed_task = task_registry.get_task(task_id)
        print(
            f"Task #{task_id}: {completed_task.get('task')} [COMPLETED][{completed_task.get('skill')}]"
        )
        ...
        if REFLECTION:
            (
                ...
                insert_after_ids,
                tasks_to_update,
            ) = task_registry.reflect_on_output(task_output, skill_descriptions)
...
def run_main_loop(OBJECTIVE, LOAD_SKILLS, api_keys, REFLECTION=False):
    ...
    try:
        ...
        task_outputs = {
            task["id"]: {"completed": False, "output": None}
        ...
        while not all(task["completed"] for task in task_outputs.values()):
            ...
            task_registry.reflect_on_final(
                OBJECTIVE, task_registry.get_tasks(), task_output, skill_descriptions
            )
            global_skill_registry.reflect_skills(
                OBJECTIVE, task_registry.get_tasks(), task_output, skill_descriptions
            )

            with open(
                f'output/output_{datetime.now().strftime("%d_%m_%Y_%H_%M_%S")}.txt', "w"
            ) as file:
...
global_skill_registry = SkillRegistry(
    api_keys=api_keys, main_loop_function=run_main_loop, skill_names=LOAD_SKILLS
)
