...
class CodeReader(Skill):
    ...
    def execute(self, params, dependent_task_outputs, objective):
        ...
        dir_structure = self.get_directory_structure(
            self.get_top_parent_path(os.path.realpath(__file__))
        )
        ...
        example_dir_structure = {
            ...
            "skills": {
                ...
                "skill.py": None,
                "test_skill.py": None,
                "text_completion.py": None,
                "web_search.py": None,
                "skill_registry.py": None,
                "directory_structure.py": None,
                "code_reader.py": None,
            },
            "tasks": {"task_registry.py": None},
            "output": {},
        }
    ...
    def get_directory_structure(self, start_path):
        ...
        ignore_dirs = [
            ...
            "poetry",
            "venv",
        ]  # add any other directories to ignore here

        for root, dirs, files in os.walk(start_path):
            dirs[:] = [
                d for d in dirs if not any(d.startswith(i) for i in ignore_dirs)
            ]  # exclude specified directories
            files = [
                f for f in files if not f[0] == "." and f.endswith(".py")
            ]  # exclude hidden files and non-Python files
