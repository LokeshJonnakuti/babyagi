    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        while not all(task["completed"] for task in task_outputs.values()):
            ...
            futures = [
                ...
                if not task_outputs[task_id]["completed"]
            ]
            ...
            for future in futures:
                ...
                task_registry.update_tasks(
                    {"id": i, "status": "completed", "result": output}
                )

                completed_task = task_registry.get_task(i)
                print(
                    f"\033[92mTask #{i}: {completed_task.get('task')} \033[0m\033[92m[COMPLETED]\033[0m\033[92m[{completed_task.get('skill')}]\033[0m"
                )
